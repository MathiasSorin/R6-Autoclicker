...
def config():
    ...
    info["text"] = "In gamemode preferences, set all to no except protect hostage, when ready press Next"

    nextstep.wait_variable(resume)
    winsound.PlaySound("clic.wav", winsound.SND_ASYNC)
    resume.set(0)

    info["text"] = "In map preferences, set all to no except Tower, when ready press Next"
